import React from 'react';
import '../Component/Header1.css';
import SearchIcon from '@material-ui/icons/Search';
import ExpandMoreIcon from '@material-ui/icons/ExpandMore';
import AddIcon from '@material-ui/icons/Add';
import { Link } from 'react-router-dom';

class Header extends React.Component {
  render() {
    return (
      <div className="mainBox">
        <div className="mainBox__logoLocation">
          <div className="mainBox__logoLocation--logo">
            <img src="data:image/png;base64," />
          </div>

          <div className="mainBox__logoLocation--location">
            <button> <SearchIcon id="btnIcon" /> </button>

            <input type="text" placeholder="Pakistan" />
            <button> <ExpandMoreIcon id="btnArrow" /> </button>
          </div>
        </div>
        <div className="mainBox__searchBar">
          <input type="text" placeholder="Find cars, Mobile phones and more..." />
          <button><SearchIcon  id="searchIcon" /></button>
        </div>
        <div className="mainBox__loginSell">
          <div className="mainBox__loginSell--login">
            <button className="login"> 
            <span >Login</span>
             </button>

          </div>
          <div className="mainBox__loginSell--sell">
            <Link  to='/postheader'>
            <button className="Sell">
              <span className="plusIcon"><AddIcon /></span>
              <span className="sellText">SELL</span>
            </button>
            </Link>
          </div>

        </div>

      </div>
    )
  }
}


export default Header;